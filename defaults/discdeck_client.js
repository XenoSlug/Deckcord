window.Vencord.Plugins.plugins.Discdeck = {
    name: "Discdeck",
    description: "Plugin required for Discdeck to work",
    authors: [],
    required: true,
    ...
    async start() {
        navigator.mediaDevices.getUserMedia = (_) => new Promise(async (resolve, reject) => {
            ...
            window.DISCDECK_WS.addEventListener("message", async (e) => {
            ...
            peerConnection.addEventListener("icecandidate", event => {
                if (event.candidate) {
                    window.DISCDECK_WS.send(JSON.stringify({ type: "$MIC_WEBRTC", ice: event.candidate }));
                }
            });
            ...
            window.DISCDECK_WS.send(JSON.stringify({ type: "$MIC_WEBRTC", offer: offer }));
        });
        ...
        console.log("Discdeck: Waiting for FluxDispatcher...");
        Vencord.Webpack.waitFor(["subscribe", "dispatch", "register"], fdm => {
            ...
            function connect() {
                window.DISCDECK_WS = new WebSocket('ws://127.0.0.1:65123/socket');
                window.DISCDECK_WS.addEventListener("message", async function (e) {
                    const data = JSON.parse(e.data);
                    if (data.type.startsWith("$")) {
                        ...
                        const payload = {
                            type: "$discdeck_request",
                            increment: data.increment,
                            result: result || {}
                        };
                        console.debug(data, payload);
                        window.DISCDECK_WS.send(JSON.stringify(payload));
                        return;
                    }
                    FluxDispatcher.dispatch(data);
                });

                window.DISCDECK_WS.onopen = function (e) {
                    navigator.mediaDevices.getUserMedia();
                    Vencord.Webpack.waitFor("useState", t =>
                        window.DISCDECK_WS.send(JSON.stringify({
                            type: "LOADED",
                            result: true
                        }))
                    );
                ...
                window.DISCDECK_WS.onclose = function (e) {
                ...
                window.DISCDECK_WS.onerror = function (err) {
                    console.error('Socket encountered error: ', err.message, 'Closing socket');
                    window.DISCDECK_WS.close();
                };
                ...
                FluxDispatcher.addInterceptor(e => {
                    ...
                    if (shouldPass) {
                        console.log("Dispatching Discdeck event: ", e);
                        window.DISCDECK_WS.send(JSON.stringify(e));
                    }
                });
                console.log("Discdeck: Added event interceptor");
            }
            connect();
        });
